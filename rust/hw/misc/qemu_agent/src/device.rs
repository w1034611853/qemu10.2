...
use hwcore::{
    DeviceImpl, DeviceState, ResettablePhasesImpl, ResetType, SysBusDevice,
    SysBusDeviceImpl, SysBusDeviceMethods,
};
use migration::{
    impl_vmstate_struct, vmstate_fields, vmstate_of, VMStateDescription, VMStateDescriptionBuilder,
};
use qom::{ObjectImpl, ObjectType, ParentField, ParentInit, qom_isa};
...
// C helper functions for address space access (not available in v10.2.0 Rust bindings).
extern "C" {
    fn qemu_agent_read_phys_u32(addr: u64, val: *mut u32) -> u32;
    fn qemu_agent_write_phys_u32(addr: u64, val: u32) -> u32;
}
...
qom_isa!(QemuAgentState: SysBusDevice, DeviceState, qom::Object);
...
impl ObjectImpl for QemuAgentState {
    ...
    const CLASS_INIT: fn(&mut Self::Class) = hwcore::SysBusDeviceClass::class_init::<Self>;
}
...
impl QemuAgentState {
    ...
    fn submit_report(&self) {
        ...
        // ---- hook for QEMU internal adjustment ----
        self.handle_report();
        // -------------------------------------------
    ...
    fn handle_report(&self) {
        ...
        match typ {
            report_types::CPU_USAGE => {
                log_mask_ln!(Log::Unimp, "qemu-agent: CPU usage report {v0}%");
            }
            report_types::MEM_USAGE => {
                log_mask_ln!(Log::Unimp, "qemu-agent: MEM usage {v0}MB / {v1}MB");
            }
    ...
    fn cmd_read_phys_u32(&self, addr: u32) -> (u32, u32) {
        let mut val: u32 = 0;
        let ret = unsafe { qemu_agent_read_phys_u32(addr as u64, &mut val) };
        if ret == 0 {
            // MEMTX_OK is 0
            (val, 0)
        } else {
    ...
    fn cmd_write_phys_u32(&self, addr: u32, value: u32) -> (u32, u32) {
        let ret = unsafe { qemu_agent_write_phys_u32(addr as u64, value) };
        (if ret == 0 { 0 } else { 1 }, 0)
    }
