...
static inline void a64_invalidate_x86_flags(DisasContext *s)
{
    /*
     * x86_cc_op is only meaningful while x86_flags_valid says the canonical
     * cache is live, so clearing the valid bit is sufficient on the hot path.
     */
...
static inline void a64_clear_adjacent_host_flags(DisasContext *s)
{
    s->a64_adj_host_flags = (A64AdjacentHostFlags) { 0 };
}

static inline void a64_record_adjacent_host_flags(DisasContext *s)
{
    s->a64_adj_host_flags.valid = true;
    s->a64_adj_host_flags.keep = true;
}
...
static inline bool a64_cc_op_carry_is_inverted(uint32_t cc_op)
{
    ...
    case A64_X86_CC_SBC32:
    case A64_X86_CC_SBC64:
        return true;
    default:
        return false;
    }
...
{
    tcg_gen_shri_i32(dst, cpu_x86_raw_flags, bit);
...
{
    tcg_debug_assert(s->a64_raw_cc_op != A64_X86_CC_INVALID);
...
static void a64_record_conditional_cc_for_bcond(DisasContext *s, bool sf,
                                                TCGv_i64 lhs, TCGv_i64 rhs,
                                                TCGv_i32 selector,
                                                TCGOp *end, uint32_t cc_op,
                                                uint8_t literal_nzcv)
{
    tcg_debug_assert(cc_op == A64_X86_CC_ADD64 ||
                     cc_op == A64_X86_CC_ADD32 ||
                     cc_op == A64_X86_CC_SUB64 ||
                     cc_op == A64_X86_CC_SUB32);
    ...
    s->a64_pending_cc.rhs = rhs;
    s->a64_pending_cc.carry = NULL;
    s->a64_pending_cc.selector = selector;
    s->a64_pending_cc.rewind = NULL;
    s->a64_pending_cc.end = end;
    s->a64_pending_cc.cc_op = cc_op;
    s->a64_pending_cc.literal_nzcv = literal_nzcv;
    s->a64_pending_cc.gap_insns = 0;
    s->a64_pending_cc.kind =
        (cc_op == A64_X86_CC_ADD64 || cc_op == A64_X86_CC_ADD32) ?
        A64_PENDING_CC_CONDITIONAL_ADD : A64_PENDING_CC_CONDITIONAL_SUB;
    s->a64_pending_cc.pc = s->pc_curr;
    s->a64_pending_cc.age = 0;
    s->a64_pending_cc.consumed = false;
    s->a64_pending_cc.consumer = NULL;
...
static bool a64_insn_is_plain_bcond(uint32_t insn, int *cc)
{
...
static bool a64_insn_is_plain_sbc_reg(uint32_t insn)
{
    /*
     * SBC . 10 11010000 ..... 000000 ..... .....
     * Restrict the first direct-consumer step to the plain register form.
     */
    return (insn & 0x7fe0fc00u) == 0x5a000000u;
}

static bool a64_insn_is_plain_adc_reg(uint32_t insn)
{
    /*
     * ADC . 00 11010000 ..... 000000 ..... .....
     * Restrict the first add-side direct-consumer step to the plain
     * register form.
     */
...
static bool a64_cmp_sbc_direct_enabled(void)
{
    ...
    if (cached < 0) {
        cached = getenv("QEMU_A64_DISABLE_CMP_SBC_DIRECT") ? 0 : 1;
    }
    return cached != 0;
}

static bool a64_add_adc_direct_enabled(void)
{
    static int cached = -1;

    if (cached < 0) {
        cached = getenv("QEMU_A64_DISABLE_ADD_ADC_DIRECT") ? 0 : 1;
    }
    return cached != 0;
}

static bool a64_sub_sbc_direct_enabled(void)
{
    static int cached = -1;

    if (cached < 0) {
        cached = getenv("QEMU_A64_DISABLE_SUB_SBC_DIRECT") ? 0 : 1;
    }
    return cached != 0;
}
...
static bool a64_insn_is_cmp_gap_safe(uint32_t insn)
{
    ...
    if (insn == 0xd503201fu) { /* NOP */
        return true;
    }

    if ((insn & 0x1f800000u) == 0x12800000u) { /* MOVN/MOVZ/MOVK */
        return true;
    }

    if ((insn & 0x1f800000u) == 0x13000000u) { /* SBFM/BFM/UBFM */
        return true;
    }

    if ((insn & 0x1f000000u) == 0x11000000u &&
        !extract32(insn, 29, 1)) { /* ADD/SUB immediate without flags */
        return true;
    }

    if ((insn & 0x1f000000u) == 0x0a000000u) { /* Logical shifted reg */
        opc = extract32(insn, 29, 2);
        if (opc != 3) {
            return true; /* AND/BIC/ORR/ORN/EOR/EON */
        }
    }

    if ((insn & 0x1f200000u) == 0x0b000000u &&
        !extract32(insn, 29, 1)) { /* ADD/SUB shifted reg without flags */
        return true;
    }

    if ((insn & 0x1fe00000u) == 0x0b200000u &&
        !extract32(insn, 29, 1)) { /* ADD/SUB extended reg without flags */
        return true;
    }
...
static bool a64_pc_has_adjacent_compare_rechain_successor(DisasContext *s,
                                                          target_ulong pc)
{
    uint32_t insn;
    int cc;
    ...
    if (a64_insn_is_condsel(insn, &cc)) {
        return a64_cmp_cond_supported_for_lazy_condsel(cc);
    }
    if (a64_insn_is_ccmp(insn, &cc)) {
...
static bool a64_find_future_condsel_rechain_gap(DisasContext *s,
                                                uint8_t *gap_insns,
                                                int *match_cc)
{
    target_ulong pc = s->base.pc_next;
    ...
    *gap_insns = 0;
    ...
    for (int gap = 0; gap < remaining && gap <= A64_CMP_PENDING_GAP_MAX;
        ...
        if (a64_insn_is_condsel(insn, &cc)) {
            ...
            if (!a64_pc_has_adjacent_compare_rechain_successor(s, pc + 4)) {
                return false;
            }
            *gap_insns = gap;
            if (match_cc) {
...
static bool a64_find_adjacent_plain_sbc_cmp(DisasContext *s)
{
    target_ulong pc = s->base.pc_next;
...
static void a64_finish_adjacent_host_flags_lifetime(DisasContext *s)
{
    if (!s->a64_adj_host_flags.valid) {
        return;
    }
    if (!s->a64_adj_host_flags.keep) {
        a64_clear_adjacent_host_flags(s);
    }
}
...
static bool a64_pending_cc_is_conditional_materialized(DisasContext *s);
...
static int a64_logic_cond_to_x86_jcc(int *jcc, int cc)
{
    switch (cc) {
    case 0: /* eq */
        *jcc = 0x4; /* je */
...
static bool a64_add_cond_to_x86_jcc(int *jcc, int cc)
{
    switch (cc) {
    case 0: /* eq */
        *jcc = 0x4; /* je */
...
static void a64_set_split_flags_from_bits(DisasContext *s, TCGv_i32 n,
                                          TCGv_i32 z, TCGv_i32 c,
                                          TCGv_i32 v)
{
...
static void a64_retire_pending_cc_to_split(DisasContext *s, TCGv_i32 n,
                                           TCGv_i32 z, TCGv_i32 c,
                                           TCGv_i32 v)
{
    tcg_debug_assert(s->a64_pending_cc.valid);
    a64_set_split_flags_from_bits(s, n, z, c, v);
    a64_cmp_note_drop(s);
    a64_clear_pending_cc_producer(s);
}
                                                     ...
                                                     TCGv_i32 v)
{
    ...
    a64_retire_pending_cc_to_split(s, n, z, c, v);
}

static bool a64_pending_cc_can_rechain_after_transparent_consumer(DisasContext *s)
{
    if (!s->a64_pending_cc.valid ||
        a64_pending_cc_has_live_split_flags(s)) {
        return false;
    ...
    switch (s->a64_pending_cc.kind) {
    case A64_PENDING_CC_REWINDABLE_CMP:
    case A64_PENDING_CC_CONDITIONAL_ADD:
    case A64_PENDING_CC_CONDITIONAL_SUB:
        break;
    default:
        return false;
    }
...
static bool a64_pending_cc_should_rechain_after_transparent_consumer(
    DisasContext *s)
{
    uint8_t gap_insns = 0;

    if (a64_pending_cc_is_conditional_materialized(s)) {
        if (a64_find_future_condsel_gap(s, &gap_insns, NULL) &&
            gap_insns == 0) {
            return true;
        }
        if (a64_find_future_ccmp_gap(s, &gap_insns, NULL) &&
            gap_insns == 0) {
            return true;
        }
        return a64_find_future_bcond_gap(s, &gap_insns, NULL) &&
               gap_insns == 0;
    }

    /*
     * First producer-rechain slices stay intentionally narrow: only restart
     * an adjacent compare-like producer when the next guest consumer is one
     * we already know how to consume directly from a fresh pending compare.
     *
     * This now includes one more flags-transparent CSEL/CS* hop, which is
     * strictly stronger than the earlier one-shot "... -> B.cond/CCMP" shape:
     * it lets the same compare-like producer survive one more non-branch
     * consumer before the eventual branch-or-CCMP endpoint.
     */
    if (a64_find_future_condsel_gap(s, &gap_insns, NULL) && gap_insns == 0) {
        return true;
    }
    if (a64_find_future_bcond_gap(s, &gap_insns, NULL) && gap_insns == 0) {
        return true;
    }
    if (a64_find_future_ccmp_gap(s, &gap_insns, NULL) && gap_insns == 0) {
        return true;
    }
    return false;
}

static bool a64_compare_like_producer_needs_adjacent_pending(DisasContext *s)
{
    uint8_t gap_insns = 0;
    ...
    return a64_find_future_ccmp_gap(s, &gap_insns, NULL) && gap_insns == 0;
}
...
static bool a64_pending_cc_is_conditional_materialized(DisasContext *s)
{
    ...
    return s->a64_pending_cc.kind == A64_PENDING_CC_CONDITIONAL_ADD ||
           s->a64_pending_cc.kind == A64_PENDING_CC_CONDITIONAL_SUB;
}

static bool a64_nzcv_literal_matches_cond(int cc, uint8_t nzcv)
{
    bool n = (nzcv >> 3) & 1;
    bool z = (nzcv >> 2) & 1;
    bool c = (nzcv >> 1) & 1;
...
static void a64_retire_current_pending_cc(DisasContext *s)
{
    if (!s->a64_pending_cc.valid) {
        return;
    }
...
static void a64_rechain_compare_like_pending_after_transparent_consumer(
    DisasContext *s, bool sf, TCGv_i64 lhs, TCGv_i64 rhs, uint32_t cc_op)
{
    /*
     * Flags-transparent consumers like CSEL/CS* do not change architectural
     * NZCV, so after retiring stable split flags for generic readers we can
     * restart a fresh adjacent-only compare producer for the very next direct
     * consumer.
     */
    a64_record_cmp_for_bcond(s, sf, lhs, rhs, NULL, tcg_last_op(), cc_op, 0);
}

static void a64_rechain_pending_cc_after_transparent_consumer(
    DisasContext *s, const A64PendingCCProducer *pending)
{
    switch (pending->kind) {
    case A64_PENDING_CC_REWINDABLE_CMP:
        a64_rechain_compare_like_pending_after_transparent_consumer(
            s, pending->sf, pending->lhs, pending->rhs, pending->cc_op);
        return;
    case A64_PENDING_CC_CONDITIONAL_ADD:
    case A64_PENDING_CC_CONDITIONAL_SUB:
        a64_record_conditional_cc_for_bcond(
            s, pending->sf, pending->lhs, pending->rhs, pending->selector,
            tcg_last_op(), pending->cc_op, pending->literal_nzcv);
        return;
    default:
        g_assert_not_reached();
    }
...
static bool a64_try_consume_conditional_cc_cond_bool_i32_to_split(
    DisasContext *s, int cc, TCGv_i32 dst, const char *consumer)
{
    TCGv_i32 calc_n, calc_z, calc_c, calc_v;
    ...
    calc_n = tcg_temp_new_i32();
    calc_z = tcg_temp_new_i32();
    calc_c = tcg_temp_new_i32();
    calc_v = tcg_temp_new_i32();
    ...
    if (s->a64_pending_cc.kind == A64_PENDING_CC_CONDITIONAL_ADD) {
        a64_gen_add_nzcv_bits(s->a64_pending_cc.sf, tcg_temp_new_i64(),
    ...
    tcg_gen_movcond_i32(TCG_COND_NE, out_n, s->a64_pending_cc.selector, zero,
                        calc_n,
                        tcg_constant_i32((s->a64_pending_cc.literal_nzcv >> 3) & 1));
    tcg_gen_movcond_i32(TCG_COND_NE, out_z, s->a64_pending_cc.selector, zero,
                        calc_z,
                        tcg_constant_i32((s->a64_pending_cc.literal_nzcv >> 2) & 1));
    tcg_gen_movcond_i32(TCG_COND_NE, out_c, s->a64_pending_cc.selector, zero,
                        calc_c,
                        tcg_constant_i32((s->a64_pending_cc.literal_nzcv >> 1) & 1));
    tcg_gen_movcond_i32(TCG_COND_NE, out_v, s->a64_pending_cc.selector, zero,
                        calc_v,
                        tcg_constant_i32(s->a64_pending_cc.literal_nzcv & 1));

    a64_set_cond_bool_from_nzcv_bits(dst, cc, out_n, out_z, out_c, out_v);
    a64_cmp_note_consume(s, consumer);
    a64_retire_pending_cc_to_split(s, out_n, out_z, out_c, out_v);
    return true;
}
...
static TCGv_i32 a64_test_cc_bool_i32_main(DisasContext *s, int cc)
{
...
static bool a64_try_emit_conditional_cc_bcond(DisasContext *s, int cc,
                                              TCGLabel *label)
{
    TCGv_i32 calc_n, calc_z, calc_c, calc_v;
    TCGv_i32 calc_match, final_match, zero;
    ...
    calc_c = tcg_temp_new_i32();
    calc_v = tcg_temp_new_i32();
    calc_match = tcg_temp_new_i32();
    final_match = tcg_temp_new_i32();
    zero = tcg_constant_i32(0);
    ...
    a64_set_cond_bool_from_nzcv_bits(calc_match, cc, calc_n, calc_z, calc_c,
                                     calc_v);
    tcg_gen_movcond_i32(TCG_COND_NE, final_match, s->a64_pending_cc.selector,
                        zero, calc_match,
                        tcg_constant_i32(
                            a64_nzcv_literal_matches_cond(
                                cc, s->a64_pending_cc.literal_nzcv)));

    a64_cmp_note_consume(s, "B.cond-CCMP-pending");
    tcg_gen_brcondi_i32(TCG_COND_NE, final_match, 0, label);
    return true;
}
...
static bool a64_adjacent_host_flags_is_adjacent_to_curr_insn(DisasContext *s)
{
    ...
    if (last == s->a64_adj_host_flags.end) {
        return true;
    }

    return last == s->base.insn_start &&
        QTAILQ_PREV(last, link) == s->a64_adj_host_flags.end;
}

/*
 * If the original compare producer is still in the IR, its split flags are
...
static void a64_emit_x86_jcc(int jcc, TCGLabel *label)
{
    TCGOp *op = tcg_emit_op(INDEX_op_x86_jcc, 2);

    tcg_set_insn_param(op, 0, jcc);
    tcg_set_insn_param(op, 1, label_arg(label));
    a64_add_label_use(label, op);
}

static void a64_emit_x86_cmov_i64(TCGv_i64 dst, TCGv_i64 src_true, int jcc)
{
    TCGOp *op = tcg_emit_op(INDEX_op_x86_cmov, 4);

    TCGOP_TYPE(op) = TCG_TYPE_I64;
    tcg_set_insn_param(op, 0, tcgv_i64_arg(dst));
    tcg_set_insn_param(op, 1, tcgv_i64_arg(src_true));
    tcg_set_insn_param(op, 2, tcgv_i64_arg(dst));
    tcg_set_insn_param(op, 3, jcc);
}

static void a64_emit_x86_movi_noflags_i64(TCGv_i64 dst, int64_t imm)
{
    TCGOp *op = tcg_emit_op(INDEX_op_x86_movi_noflags, 2);

    TCGOP_TYPE(op) = TCG_TYPE_I64;
    tcg_set_insn_param(op, 0, tcgv_i64_arg(dst));
    tcg_set_insn_param(op, 1, imm);
}

static void a64_emit_x86_addi_noflags_i64(TCGv_i64 dst, TCGv_i64 lhs,
                                          int64_t imm)
{
    TCGOp *op = tcg_emit_op(INDEX_op_x86_add_noflags, 3);

    TCGOP_TYPE(op) = TCG_TYPE_I64;
    tcg_set_insn_param(op, 0, tcgv_i64_arg(dst));
    tcg_set_insn_param(op, 1, tcgv_i64_arg(lhs));
    tcg_set_insn_param(op, 2, tcgv_i64_arg(tcg_constant_i64(imm)));
}

static bool a64_tcgv_i64_is_const(TCGv_i64 value)
{
    return tcgv_i64_temp(value)->kind == TEMP_CONST;
}

static int64_t a64_tcgv_i64_const_value(TCGv_i64 value)
{
    TCGTemp *temp = tcgv_i64_temp(value);

    tcg_debug_assert(temp->kind == TEMP_CONST);
    return temp->val;
}
...
static void a64_emit_addci_i64(TCGv_i64 dst, TCGv_i64 lhs, TCGv_i64 rhs)
{
    TCGOp *op = tcg_emit_op(INDEX_op_addci, 3);
...
static void a64_emit_addci_i32(TCGv_i32 dst, TCGv_i32 lhs, TCGv_i32 rhs)
{
    TCGOp *op = tcg_emit_op(INDEX_op_addci, 3);
...
static void a64_emit_subbio_i64(TCGv_i64 dst, TCGv_i64 lhs, TCGv_i64 rhs)
{
    TCGOp *op;

    g_assert(dst == lhs);
    op = tcg_emit_op(INDEX_op_subbio, 3);
    TCGOP_TYPE(op) = TCG_TYPE_I64;
    tcg_set_insn_param(op, 0, tcgv_i64_arg(dst));
    tcg_set_insn_param(op, 1, tcgv_i64_arg(lhs));
    tcg_set_insn_param(op, 2, tcgv_i64_arg(rhs));
...
static void a64_emit_subbio_i32(TCGv_i32 dst, TCGv_i32 lhs, TCGv_i32 rhs)
{
    TCGOp *op;
...
static void a64_emit_subbi_i64(TCGv_i64 dst, TCGv_i64 lhs, TCGv_i64 rhs)
{
    tcg_gen_mov_i64(dst, lhs);
    a64_emit_subbio_i64(dst, dst, rhs);
}

static void a64_emit_subbi_i32(TCGv_i32 dst, TCGv_i32 lhs, TCGv_i32 rhs)
{
    tcg_gen_mov_i32(dst, lhs);
    a64_emit_subbio_i32(dst, dst, rhs);
}
...
static void a64_emit_x86_sbb_capture_rawflags_i64(TCGv_i64 dst, TCGv_i64 lhs,
                                                  TCGv_i64 rhs,
                                                  TCGv_i32 carry_in,
                                                  TCGv_i32 raw)
{
    TCGOp *op;
    TCGv_i32 raw_tmp = tcg_temp_new_i32();

    tcg_gen_mov_i64(dst, lhs);
    op = tcg_emit_op(INDEX_op_x86_sbb_capture_rawflags, 5);
    TCGOP_TYPE(op) = TCG_TYPE_I64;
    tcg_set_insn_param(op, 0, tcgv_i32_arg(raw_tmp));
    tcg_set_insn_param(op, 1, tcgv_i64_arg(dst));
    tcg_set_insn_param(op, 2, tcgv_i64_arg(dst));
    tcg_set_insn_param(op, 3, tcgv_i64_arg(rhs));
    tcg_set_insn_param(op, 4, tcgv_i32_arg(carry_in));
    tcg_gen_mov_i32(cpu_x86_raw_flags, raw_tmp);
    tcg_gen_mov_i32(raw, raw_tmp);
}

static void a64_emit_x86_sbb_capture_rawflags_i32(TCGv_i32 dst, TCGv_i32 lhs,
                                                  TCGv_i32 rhs,
                                                  TCGv_i32 carry_in,
                                                  TCGv_i32 raw)
{
                                         ...
                                         TCGv_i64 lhs, TCGv_i64 rhs)
{
    if (sf) {
        ...
        tcg_gen_and_i64(result, lhs, rhs);
        ...
        tcg_gen_extrl_i64_i32(lhs32, lhs);
        tcg_gen_extrl_i64_i32(rhs32, rhs);
        tcg_gen_and_i32(result32, lhs32, rhs32);
        a64_emit_test_capture_rawflags_i32(lhs32, rhs32);
        a64_set_raw_flags_state(s, A64_X86_CC_LOGIC32);
    }
    a64_record_adjacent_host_flags(s);
    return true;
}
...
static void a64_emit_x86_add_hi_jcc_capture_rawflags_i64(TCGv_i64 lhs,
                                                         TCGv_i64 rhs,
                                                         TCGLabel *label)
{
    TCGv_i64 dst = tcg_temp_new_i64();
    ...
    tcg_gen_mov_i64(dst, lhs);
    op = tcg_emit_op(INDEX_op_x86_add_hi_jcc_capture_rawflags, 5);
    TCGOP_TYPE(op) = TCG_TYPE_I64;
    tcg_set_insn_param(op, 0, tcgv_i64_arg(dst));
    tcg_set_insn_param(op, 1, tcgv_i64_arg(rhs));
    tcg_set_insn_param(op, 2, 0);
    tcg_set_insn_param(op, 3, label_arg(label));
    tcg_set_insn_param(op, 4, offsetof(CPUARMState, x86_raw_flags));
    a64_add_label_use(label, op);
}

static void a64_emit_x86_add_hi_jcc_capture_rawflags_i32(TCGv_i32 lhs,
                                                         TCGv_i32 rhs,
                                                         TCGLabel *label)
{
    TCGv_i32 dst = tcg_temp_new_i32();
...
static void a64_emit_x86_add_ls_jcc_capture_rawflags_i64(TCGv_i64 lhs,
                                                         TCGv_i64 rhs,
                                                         TCGLabel *label)
{
    TCGv_i64 dst = tcg_temp_new_i64();
    ...
    tcg_gen_mov_i64(dst, lhs);
    op = tcg_emit_op(INDEX_op_x86_add_ls_jcc_capture_rawflags, 5);
    TCGOP_TYPE(op) = TCG_TYPE_I64;
    tcg_set_insn_param(op, 0, tcgv_i64_arg(dst));
    tcg_set_insn_param(op, 1, tcgv_i64_arg(rhs));
    tcg_set_insn_param(op, 2, 0);
...
static void a64_emit_x86_add_ls_jcc_capture_rawflags_i32(TCGv_i32 lhs,
                                                         TCGv_i32 rhs,
                                                         TCGLabel *label)
{
    TCGv_i32 dst = tcg_temp_new_i32();
...
static void a64_emit_x86_adc_hi_jcc_capture_rawflags_i64(TCGv_i64 lhs,
                                                         TCGv_i64 rhs,
                                                         TCGv_i32 carry,
                                                         TCGLabel *label)
{
    ...
    tcg_gen_mov_i64(dst, lhs);
    op = tcg_emit_op(INDEX_op_x86_adc_hi_jcc_capture_rawflags, 6);
    TCGOP_TYPE(op) = TCG_TYPE_I64;
    tcg_set_insn_param(op, 0, tcgv_i64_arg(dst));
    tcg_set_insn_param(op, 1, tcgv_i64_arg(rhs));
    tcg_set_insn_param(op, 2, tcgv_i32_arg(carry));
    tcg_set_insn_param(op, 3, 0);
    tcg_set_insn_param(op, 4, label_arg(label));
    tcg_set_insn_param(op, 5, offsetof(CPUARMState, x86_raw_flags));
    a64_add_label_use(label, op);
}

static void a64_emit_x86_adc_hi_jcc_capture_rawflags_i32(TCGv_i32 lhs,
                                                         TCGv_i32 rhs,
                                                         TCGv_i32 carry,
                                                         TCGLabel *label)
{
...
static void a64_emit_x86_adc_ls_jcc_capture_rawflags_i64(TCGv_i64 lhs,
                                                         TCGv_i64 rhs,
                                                         TCGv_i32 carry,
                                                         TCGLabel *label)
{
    ...
    tcg_gen_mov_i64(dst, lhs);
    op = tcg_emit_op(INDEX_op_x86_adc_ls_jcc_capture_rawflags, 6);
    TCGOP_TYPE(op) = TCG_TYPE_I64;
    tcg_set_insn_param(op, 0, tcgv_i64_arg(dst));
    tcg_set_insn_param(op, 1, tcgv_i64_arg(rhs));
    tcg_set_insn_param(op, 2, tcgv_i32_arg(carry));
...
static void a64_emit_x86_adc_ls_jcc_capture_rawflags_i32(TCGv_i32 lhs,
                                                         TCGv_i32 rhs,
                                                         TCGv_i32 carry,
                                                         TCGLabel *label)
{
...
static void a64_emit_x86_cmp_sbb_capture_cmp_rawflags_i64(TCGv_i64 dst,
                                                          ...
                                                          TCGv_i64 cmp_rhs)
{
    TCGOp *op = tcg_emit_op(INDEX_op_x86_cmp_sbb_capture_cmp_rawflags, 6);

    TCGOP_TYPE(op) = TCG_TYPE_I64;
    tcg_set_insn_param(op, 0, tcgv_i64_arg(dst));
    tcg_set_insn_param(op, 1, tcgv_i64_arg(dst));
    tcg_set_insn_param(op, 2, tcgv_i64_arg(src));
    tcg_set_insn_param(op, 3, tcgv_i64_arg(cmp_lhs));
    tcg_set_insn_param(op, 4, tcgv_i64_arg(cmp_rhs));
    tcg_set_insn_param(op, 5, offsetof(CPUARMState, x86_raw_flags));
}

static void a64_emit_x86_cmp_sbb_capture_cmp_rawflags_i32(TCGv_i32 dst,
                                                          TCGv_i32 src,
                                                          TCGv_i32 cmp_lhs,
                                                          TCGv_i32 cmp_rhs)
{
    ...
    tcg_set_insn_param(op, 0, tcgv_i32_arg(dst));
    tcg_set_insn_param(op, 1, tcgv_i32_arg(dst));
    tcg_set_insn_param(op, 2, tcgv_i32_arg(src));
    tcg_set_insn_param(op, 3, tcgv_i32_arg(cmp_lhs));
    tcg_set_insn_param(op, 4, tcgv_i32_arg(cmp_rhs));
    tcg_set_insn_param(op, 5, offsetof(CPUARMState, x86_raw_flags));
}

static void a64_emit_x86_add_adc_capture_add_rawflags_i64(TCGv_i64 dst,
                                                          TCGv_i64 adc_lhs,
                                                          TCGv_i64 adc_rhs,
                                                          TCGv_i64 add_lhs,
                                                          TCGv_i64 add_rhs)
{
    TCGOp *op = tcg_emit_op(INDEX_op_x86_add_adc_capture_add_rawflags, 6);

    TCGOP_TYPE(op) = TCG_TYPE_I64;
    tcg_set_insn_param(op, 0, tcgv_i64_arg(dst));
    tcg_set_insn_param(op, 1, tcgv_i64_arg(adc_lhs));
    tcg_set_insn_param(op, 2, tcgv_i64_arg(adc_rhs));
    tcg_set_insn_param(op, 3, tcgv_i64_arg(add_lhs));
    tcg_set_insn_param(op, 4, tcgv_i64_arg(add_rhs));
    tcg_set_insn_param(op, 5, offsetof(CPUARMState, x86_raw_flags));
}

static void a64_emit_x86_add_adc_capture_add_rawflags_i32(TCGv_i32 dst,
                                                          TCGv_i32 adc_lhs,
                                                          TCGv_i32 adc_rhs,
                                                          TCGv_i32 add_lhs,
                                                          TCGv_i32 add_rhs)
{
    TCGOp *op = tcg_emit_op(INDEX_op_x86_add_adc_capture_add_rawflags_i32, 6);

    TCGOP_TYPE(op) = TCG_TYPE_I32;
    tcg_set_insn_param(op, 0, tcgv_i32_arg(dst));
    tcg_set_insn_param(op, 1, tcgv_i32_arg(adc_lhs));
    tcg_set_insn_param(op, 2, tcgv_i32_arg(adc_rhs));
    tcg_set_insn_param(op, 3, tcgv_i32_arg(add_lhs));
    tcg_set_insn_param(op, 4, tcgv_i32_arg(add_rhs));
    tcg_set_insn_param(op, 5, offsetof(CPUARMState, x86_raw_flags));
}
                                       ...
                                       DisasLabel match, int imm)
{
    ...
    bool use_add_hi_jcc = false;
    bool use_add_ls_jcc = false;
    ...
    if (add_like || adc_like) {
        if (a64_add_cond_to_x86_jcc(&jcc, cc)) {
            use_x86_jcc = true;
            if (cc == 2 || cc == 3) {
                use_tcg_cond = false;
            }
    ...
    if (adc_like && s->a64_pending_cc.sf) {
        if (use_add_hi_jcc) {
            a64_emit_x86_adc_hi_jcc_capture_rawflags_i64(
                s->a64_pending_cc.lhs, s->a64_pending_cc.rhs,
                s->a64_pending_cc.carry, match.label);
        } else if (use_add_ls_jcc) {
            a64_emit_x86_adc_ls_jcc_capture_rawflags_i64(
                s->a64_pending_cc.lhs, s->a64_pending_cc.rhs,
                s->a64_pending_cc.carry, match.label);
        } else if (use_tcg_cond) {
        ...
        tcg_gen_extrl_i64_i32(rhs, s->a64_pending_cc.rhs);
        if (use_add_hi_jcc) {
            a64_emit_x86_adc_hi_jcc_capture_rawflags_i32(
                lhs, rhs, s->a64_pending_cc.carry, match.label);
        } else if (use_add_ls_jcc) {
            a64_emit_x86_adc_ls_jcc_capture_rawflags_i32(
                lhs, rhs, s->a64_pending_cc.carry, match.label);
        } else if (use_tcg_cond) {
        ...
        if (use_add_hi_jcc) {
            a64_emit_x86_add_hi_jcc_capture_rawflags_i64(
                s->a64_pending_cc.lhs, s->a64_pending_cc.rhs,
                match.label);
        } else if (use_add_ls_jcc) {
            a64_emit_x86_add_ls_jcc_capture_rawflags_i64(
                s->a64_pending_cc.lhs, s->a64_pending_cc.rhs,
                match.label);
        } else if (use_tcg_cond) {
        ...
        tcg_gen_extrl_i64_i32(rhs, s->a64_pending_cc.rhs);
        if (use_add_hi_jcc) {
            a64_emit_x86_add_hi_jcc_capture_rawflags_i32(lhs, rhs,
                                                         match.label);
        } else if (use_add_ls_jcc) {
            a64_emit_x86_add_ls_jcc_capture_rawflags_i32(lhs, rhs,
                                                         match.label);
        } else if (use_tcg_cond) {
...
static bool a64_try_emit_x86_logic_bcond(DisasContext *s, int cc,
                                         DisasLabel match, int imm)
{
    int jcc;
    int kind;
    ...
    if (!s->a64_adj_host_flags.valid ||
        !a64_adjacent_host_flags_is_adjacent_to_curr_insn(s) ||
        s->a64_flags_rep != A64_FLAGS_REP_RAW ||
        (s->a64_raw_cc_op != A64_X86_CC_LOGIC64 &&
         s->a64_raw_cc_op != A64_X86_CC_LOGIC32)) {
        return false;
    }

    kind = a64_logic_cond_to_x86_jcc(&jcc, cc);
    if (kind == 0) {
        return false;
    }
    ...
    a64_emit_x86_jcc(jcc, match.label);
    gen_goto_tb(s, 0, 4);
    set_disas_label(s, match);
    gen_goto_tb(s, 1, imm);
    return true;
...
static bool a64_logic_csel_is_host_direct_safe(const arg_CSEL *a)
{
    (void)a;
    return true;
}

static void a64_emit_ccmp_literal_raw_flags(DisasContext *s, uint32_t nzcv)
{
    a64_write_raw_flags_from_bits(s,
                                  tcg_constant_i32((nzcv >> 3) & 1),
                                  tcg_constant_i32((nzcv >> 2) & 1),
                                  tcg_constant_i32((nzcv >> 1) & 1),
                                  tcg_constant_i32(nzcv & 1));
}

static void a64_emit_ccmp_calc_raw_flags(DisasContext *s, const arg_CCMP *a,
                                         TCGv_i64 tcg_rn, TCGv_i64 tcg_y)
{
    ...
    a64_write_raw_flags_from_bits(s, calc_n, calc_z, calc_c, calc_v);
}

static bool a64_try_emit_x86_logic_ccmp(DisasContext *s, arg_CCMP *a)
{
    ...
    kind = a64_logic_cond_to_x86_jcc(&jcc, a->cond);
    if (kind == 0) {
        return false;
    }
    ...
    if (kind < 0) {
        a64_emit_ccmp_literal_raw_flags(s, a->nzcv);
        return true;
    }
    if (kind > 1) {
        a64_emit_ccmp_calc_raw_flags(s, a, tcg_rn, tcg_y);
        return true;
    }

    {
        ...
        a64_emit_x86_jcc(jcc, label_true);
        a64_emit_ccmp_literal_raw_flags(s, a->nzcv);
        tcg_gen_br(label_done);

        gen_set_label(label_true);
        a64_emit_ccmp_calc_raw_flags(s, a, tcg_rn, tcg_y);
...
static bool a64_try_emit_x86_logic_csel(DisasContext *s, arg_CSEL *a,
                                        TCGv_i64 tcg_rd)
{
    TCGv_i64 t_true;
    TCGv_i64 t_false;
    ...
    int jcc;
    int kind;
    bool cset_like = a->rn == 31 && a->rm == 31 &&
                     (a->else_inc ^ a->else_inv);
    bool csinc_like = a->else_inc && !a->else_inv && !cset_like;
    bool csneg_like = a->else_inc && a->else_inv;

    if (!a64_logic_csel_is_host_direct_safe(a) ||
        !s->a64_adj_host_flags.valid ||
        !a64_adjacent_host_flags_is_adjacent_to_curr_insn(s) ||
        s->a64_flags_rep != A64_FLAGS_REP_RAW ||
        (s->a64_raw_cc_op != A64_X86_CC_LOGIC64 &&
         s->a64_raw_cc_op != A64_X86_CC_LOGIC32)) {
    ...
    if (cset_like) {
        /*
         * CSET/CSETM are aliases that encode the inverted condition in the
         * underlying CSINC/CSINV instruction.
         */
        cc ^= 1;
    }
    kind = a64_logic_cond_to_x86_jcc(&jcc, cc);
    ...
    if (!cset_like) {
        /*
         * Preserve the true-path source before writing the false path into rd.
         * Otherwise rd == rn aliases would self-clobber.
         */
        if (a->rn == 31) {
            t_true = tcg_temp_new_i64();
            a64_emit_x86_movi_noflags_i64(t_true, 0);
        } else {
            t_true = read_cpu_reg(s, a->rn, a->sf);
        }
    }

    if (cset_like) {
        a64_emit_x86_movi_noflags_i64(tcg_rd, 0);
        t_true = tcg_temp_new_i64();
        a64_emit_x86_movi_noflags_i64(t_true, a->else_inv ? -1 : 1);
    } else if (csinc_like) {
        if (a->rm == 31) {
            t_false = tcg_temp_new_i64();
            a64_emit_x86_movi_noflags_i64(t_false, 0);
        } else {
            t_false = read_cpu_reg(s, a->rm, a->sf);
        }

        a64_emit_x86_addi_noflags_i64(tcg_rd, t_false, 1);
    } else if (csneg_like) {
        if (a->rm == 31) {
            t_false = tcg_temp_new_i64();
            a64_emit_x86_movi_noflags_i64(t_false, 0);
        ...
        tcg_gen_not_i64(tcg_rd, t_false);
        a64_emit_x86_addi_noflags_i64(tcg_rd, tcg_rd, 1);
    } else {
        if (a->rm == 31) {
            t_false = tcg_temp_new_i64();
            a64_emit_x86_movi_noflags_i64(t_false, 0);
        ...
        tcg_gen_mov_i64(tcg_rd, t_false);
        if (a->else_inv) {
    ...
    if (kind > 1) {
        tcg_gen_mov_i64(tcg_rd, t_true);
    } else if (kind == 1) {
        a64_emit_x86_cmov_i64(tcg_rd, t_true, jcc);
    }
                                     ...
                                     TCGv_i64 tcg_rm)
{
    ...
    if (s->a64_pending_cc.kind == A64_PENDING_CC_MATERIALIZED_SUB) {
        ...
        a64_cmp_note_consume(s, "SBC-x86-sub-sbb");
        if (sf) {
        ...
        a64_cmp_note_consume(s, "SBC-x86-cmp-sbb");
        if (sf) {
...
#if defined(__i386__) || defined(__x86_64__)
static bool a64_try_emit_x86_add_adc(DisasContext *s, bool sf,
                                     TCGv_i64 tcg_rd, TCGv_i64 tcg_rn,
                                     TCGv_i64 tcg_rm)
{
    ...
    if (s->a64_pending_cc.kind == A64_PENDING_CC_MATERIALIZED_ADD) {
        ...
        if (a64_tcgv_i64_is_const(s->a64_pending_cc.rhs)) {
            ...
            if (sf) {
                TCGv_i64 producer = tcg_temp_new_i64();

                a64_emit_addco_i64(producer, s->a64_pending_cc.lhs,
                                   s->a64_pending_cc.rhs);
                ...
                int32_t imm32 =
                    (int32_t)a64_tcgv_i64_const_value(s->a64_pending_cc.rhs);
                TCGv_i32 producer = tcg_temp_new_i32();
                TCGv_i32 lhs32 = tcg_temp_new_i32();
                TCGv_i32 rn32 = tcg_temp_new_i32();
                TCGv_i32 rm32 = tcg_temp_new_i32();
                TCGv_i32 result32 = tcg_temp_new_i32();
                ...
                tcg_gen_extrl_i64_i32(rn32, tcg_rn);
                tcg_gen_extrl_i64_i32(rm32, tcg_rm);
                a64_emit_addco_i32(producer, lhs32, tcg_constant_i32(imm32));
                a64_emit_x86_capture_rawflags(raw);
                tcg_gen_mov_i32(cpu_x86_raw_flags, raw);
                a64_emit_addci_i32(result32, rn32, rm32);
                tcg_gen_extu_i32_i64(tcg_rd, result32);
...
#if defined(__i386__) || defined(__x86_64__)
/*
 * a64_try_emit_x86_add_adcs - setflags add-side consumer helper for ADCS
 *
 * Emits host adc for ADCS, consuming the pending producer's live CF directly
 * WITHOUT calling a64_get_current_carry_flag(). The key difference from plain
 * ADC is:
                                      ...
                                      TCGv_i64 tcg_rm)
{
    ...
    /* Check pending_cc is valid and is an add-type operation */
    if (!s->a64_pending_cc.valid ||
        s->a64_pending_cc.rewind != NULL ||
        !a64_pending_cc_is_adjacent_to_curr_insn(s) ||
    ...
    a64_cmp_note_consume(s, "ADCS-x86-add-adcs");
...
static void gen_add64_CC_split(TCGv_i64 dest, TCGv_i64 t0, TCGv_i64 t1)
{
    TCGv_i64 result = tcg_temp_new_i64();
    TCGv_i64 flag = tcg_temp_new_i64();
    TCGv_i64 tmp = tcg_temp_new_i64();
...
static void gen_add32_CC_split(TCGv_i64 dest, TCGv_i64 t0, TCGv_i64 t1)
{
    TCGv_i32 t0_32 = tcg_temp_new_i32();
    TCGv_i32 t1_32 = tcg_temp_new_i32();
    TCGv_i32 tmp = tcg_temp_new_i32();
...
static void gen_sub64_CC_split(TCGv_i64 dest, TCGv_i64 t0, TCGv_i64 t1)
{
    TCGv_i64 result = tcg_temp_new_i64();
    TCGv_i64 flag = tcg_temp_new_i64();
    TCGv_i64 tmp = tcg_temp_new_i64();
...
static void gen_sub32_CC_split(TCGv_i64 dest, TCGv_i64 t0, TCGv_i64 t1)
{
    TCGv_i32 t0_32 = tcg_temp_new_i32();
    TCGv_i32 t1_32 = tcg_temp_new_i32();
    TCGv_i32 tmp = tcg_temp_new_i32();
...
static void gen_adc_CC_split(int sf, TCGv_i64 dest, TCGv_i64 t0,
                             TCGv_i64 t1, TCGv_i32 carry_in)
{
    if (sf) {
        TCGv_i64 result = tcg_temp_new_i64();
        ...
        tcg_gen_extu_i32_i64(cf_64, carry_in);
        ...
        tcg_gen_extrl_i64_i32(t0_32, t0);
        tcg_gen_extrl_i64_i32(t1_32, t1);
        tcg_gen_addcio_i32(cpu_NF, cpu_CF, t0_32, t1_32, carry_in);
                       ...
                       TCGv_i64 t0, TCGv_i64 t1, bool allow_direct)
{
    ...
    a64_note_flags_split(s);
    if (sf) {
        gen_add64_CC_split(dest, t0, t1);
    } else {
        gen_add32_CC_split(dest, t0, t1);
    }
}
                       ...
                       TCGv_i64 t0, TCGv_i64 t1, bool allow_direct)
{
    ...
    a64_note_flags_split(s);
    if (sf) {
        gen_sub64_CC_split(dest, t0, t1);
    } else {
        gen_sub32_CC_split(dest, t0, t1);
    }
}

static void a64_publish_addsub_main_flags(DisasContext *s, bool sf,
                                          TCGv_i64 lhs, TCGv_i64 rhs,
                                          bool sub_op)
{
    ...
    if (sub_op) {
        gen_sub_CC(s, sf, discarded, lhs, rhs, true);
    } else {
        gen_add_CC(s, sf, discarded, lhs, rhs, true);
    }
}

/* dest = T0 + T1 + carry_in; do not compute flags. */
static void gen_adc(int sf, TCGv_i64 dest,
...
/* dest = T0 - T1 - !carry_in; do not compute flags. */
static void gen_sbc(int sf, TCGv_i64 dest,
                    TCGv_i64 t0, TCGv_i64 t1, TCGv_i32 carry_in)
{
#if defined(__i386__) || defined(__x86_64__)
    TCGv_i32 borrow_in = tcg_temp_new_i32();

    tcg_gen_xori_i32(borrow_in, carry_in, 1);
    if (sf) {
        TCGv_i64 bi64 = tcg_temp_new_i64();
        TCGv_i64 discard = tcg_temp_new_i64();

        tcg_gen_extu_i32_i64(bi64, borrow_in);
        tcg_gen_mov_i64(dest, t0);
        a64_emit_subbo_i64(discard, tcg_constant_i64(0), bi64);
        a64_emit_subbio_i64(dest, dest, t1);
        return;
    } else {
        TCGv_i32 t0_32 = tcg_temp_new_i32();
        TCGv_i32 t1_32 = tcg_temp_new_i32();
        ...
        tcg_gen_extrl_i64_i32(t1_32, t1);
        tcg_gen_mov_i32(result, t0_32);
        a64_emit_subbo_i32(discard, tcg_constant_i32(0), borrow_in);
        a64_emit_subbio_i32(result, result, t1_32);
        tcg_gen_extu_i32_i64(dest, result);
        return;
    }
#endif
    {
        TCGv_i64 flag = tcg_temp_new_i64();
        TCGv_i64 not_t1 = tcg_temp_new_i64();

        tcg_gen_extu_i32_i64(flag, carry_in);
        tcg_gen_not_i64(not_t1, t1);
        tcg_gen_add_i64(dest, t0, not_t1);
        tcg_gen_add_i64(dest, dest, flag);
                       ...
                       bool is_sub, bool allow_direct)
{
    ...
    if (allow_direct) {
        if (sf) {
            ...
            if (is_sub) {
                a64_emit_x86_sbb_capture_rawflags_i64(result, t0, t1,
                                                      carry_in, raw);
            } else {
                TCGv_i64 ci64 = tcg_temp_new_i64();
                TCGv_i64 discard = tcg_temp_new_i64();
            ...
            tcg_gen_extrl_i64_i32(t1_32, t1);
            if (is_sub) {
                a64_emit_x86_sbb_capture_rawflags_i32(result, t0_32, t1_32,
                                                      carry_in, raw);
            } else {
                TCGv_i32 discard = tcg_temp_new_i32();
    ...
    if (is_sub) {
        ...
        tcg_gen_not_i64(not_t1, t1);
        gen_adc_CC_split(sf, dest, t0, not_t1, carry_in);
    } else {
                                ...
                                TCGv_i32 carry_in)
{
    ...
    if (sf) {
        ...
        tcg_gen_extu_i32_i64(borrow64, borrow_in);
        tcg_gen_add_i64(out, rhs, borrow64);
    } else {
        TCGv_i32 rhs32 = tcg_temp_new_i32();
        TCGv_i32 out32 = tcg_temp_new_i32();

        tcg_gen_extrl_i64_i32(rhs32, rhs);
        tcg_gen_add_i32(out32, rhs32, borrow_in);
        tcg_gen_extu_i32_i64(out, out32);
    }
}
...
static bool trans_B_cond(DisasContext *s, arg_B_cond *a)
{
    ...
    if (a->cond < 0x0e) {
        ...
        if (a64_try_emit_x86_logic_bcond(s, a->cond, match, a->imm)) {
            return true;
        }
        if (a64_try_emit_x86_cmp_bcond(s, a->cond, match, a->imm)) {
            return true;
        }
        reset_btype(s);
        if (!a64_try_emit_cmp_bcond(s, a->cond, match.label)) {
            a64_gen_test_cc(s, a->cond, match.label);
        }
        gen_goto_tb(s, 0, 4);
        set_disas_label(s, match);
        gen_goto_tb(s, 1, a->imm);
...
static void gen_get_nzcv_split(TCGv_i32 nzcv)
{
    TCGv_i32 tmp = tcg_temp_new_i32();
...
static void gen_get_nzcv_raw(DisasContext *s, TCGv_i32 nzcv)
{
    ...
    tcg_gen_andi_i32(nzcv, cpu_x86_raw_flags, 0x8000);
    tcg_gen_shli_i32(nzcv, nzcv, 16);
...
static void gen_get_nzcv(DisasContext *s, TCGv_i64 tcg_rt)
{
                       ...
                       unsigned int crn, unsigned int crm, unsigned int rt)
{
    ...
    case ARM_CP_NZCV:
        tcg_rt = cpu_reg(s, rt);
        if (isread) {
            gen_get_nzcv(s, tcg_rt);
        } else {
            gen_set_nzcv(s, tcg_rt);
        }
        return;
...
static bool do_addsub_imm(DisasContext *s, arg_rri_sf *a,
                          bool sub_op, bool setflags)
{
    ...
    bool lazy_condsel_rechain_cmp = false;
    bool materialized_sbc_sub = false;
    bool materialized_adc_add = false;
    bool compare_like = setflags && a->rd == 31;
    bool need_pending_record = false;
    bool need_main_flags_publish = false;
    uint8_t gap_insns = 0;
    uint8_t condsel_rechain_gap_insns = 0;
    ...
    if (setflags && sub_op && a->rd == 31) {
        lazy_condsel_rechain_cmp =
            a64_find_future_condsel_rechain_gap(s,
                                                &condsel_rechain_gap_insns,
                                                NULL);
    ...
    need_pending_record =
        ((compare_like && a64_compare_like_producer_needs_adjacent_pending(s)) ||
         lazy_condsel_rechain_cmp ||
         lazy_fcsel_cmp ||
         lazy_sbc_cmp || lazy_adc_add ||
         materialized_sbc_sub || materialized_adc_add);
    need_main_flags_publish =
        ((lazy_bcond_cmp && gap_insns > 0) ||
         lazy_condsel_cmp || lazy_ccmp_cmp || lazy_fccmp_cmp);
    ...
    if (!setflags) {
        if (sub_op) {
            tcg_gen_sub_i64(tcg_result, tcg_rn, tcg_imm);
        } else {
    ...
    if (lazy_sbc_cmp) {
        tcg_pending_imm = tcg_temp_new_i64();
        tcg_gen_movi_i64(tcg_pending_imm, a->imm);
    }

    if (need_pending_record) {
        A64PendingCCProducerKind kind = A64_PENDING_CC_REWINDABLE_CMP;

        a64_record_cmp_for_bcond(s, a->sf, tcg_rn, tcg_pending_imm,
                                 NULL,
                                 tcg_last_op(),
 ...
 */
TRANS(ADD_i, do_addsub_imm, a, false, false)
TRANS(SUB_i, do_addsub_imm, a, true, false)
TRANS(ADDS_i, do_addsub_imm, a, false, true)
TRANS(SUBS_i, do_addsub_imm, a, true, true)
...
#if defined(__i386__) || defined(__x86_64__)
    if (set_cc &&
        fn == tcg_gen_andi_i64 && a64_logic_imm_can_direct(a->sf, imm)) {
...
static bool trans_FCCMP(DisasContext *s, arg_FCCMP *a)
{
    ...
    if (a->cond < 0x0e) { /* not always */
        ...
        TCGv_i32 cond = a64_test_cc_bool_i32_main(s, a->cond);
                       ...
                       bool is_sub, bool setflags)
{
    ...
    if (!setflags && is_sub &&
        a64_try_emit_x86_cmp_sbc(s, a->sf, tcg_rd, tcg_rn, tcg_rm)) {
        return true;
    }
    if (!setflags && !is_sub &&
        a64_try_emit_x86_add_adc(s, a->sf, tcg_rd, tcg_rn, tcg_rm)) {
        return true;
    }
...
{
    bool keep_ccmp_pending = false;
    uint8_t gap_insns = 0;

    if (a64_try_emit_x86_logic_ccmp(s, a)) {
        return true;
    }

    if ((a64_find_future_bcond_gap(s, &gap_insns, NULL) && gap_insns == 0) ||
        (a64_find_future_condsel_gap(s, &gap_insns, NULL) && gap_insns == 0) ||
        (a64_find_future_ccmp_gap(s, &gap_insns, NULL) && gap_insns == 0)) {
        keep_ccmp_pending = true;
    }

    {
        TCGv_i32 cond_true = a64_test_cc_bool_i32_main(s, a->cond);
        TCGv_i32 calc_n = tcg_temp_new_i32();
        TCGv_i32 calc_z = tcg_temp_new_i32();
        TCGv_i32 calc_c = tcg_temp_new_i32();
        TCGv_i32 calc_v = tcg_temp_new_i32();
    ...
    /* Load the arguments for the new comparison.  */
        ...
        /* Set T0 = !COND.  */
            tcg_gen_xori_i32(tcg_t0, cond_true, 1);
         ...
         */
            ...
            if (keep_ccmp_pending) {
                /*
                 * Keep one more conditional producer hop for adjacent
                 * direct-consumer chains, but also publish a durable split
                 * representation so generic readers never fall back to the
                 * UNKNOWN runtime-dispatch path on a miss.
                 */
                if (s->a64_pending_cc.valid && s->a64_pending_cc.consumed) {
                    a64_retire_current_pending_cc(s);
                }
                a64_set_split_flags_from_bits(s, out_n, out_z, out_c, out_v);
                a64_record_conditional_cc_for_bcond(
                    s, a->sf, tcg_rn, tcg_y, cond_true, tcg_last_op(),
                    a->op ? (a->sf ? A64_X86_CC_SUB64 : A64_X86_CC_SUB32)
                          : (a->sf ? A64_X86_CC_ADD64 : A64_X86_CC_ADD32),
                    a->nzcv);
                return true;
            }
...
static bool trans_CSEL(DisasContext *s, arg_CSEL *a)
{
    ...
    if (a64_try_emit_x86_logic_csel(s, a, tcg_rd)) {
        return true;
    }

    if (a64_pending_cc_can_rechain_after_transparent_consumer(s)) {
        /*
         * First slice of the new producer model: after a flags-transparent
         * compare consumer, re-seed an adjacent-only pending producer only
         * when the very next guest insn is a direct consumer shape we already
         * know how to consume from a fresh pending producer. The current
         * chain shapes are:
         *   CSEL/CS* -> CSEL/CS*
         *   CSEL/CS* -> B.cond
         *   CSEL/CS* -> CCMP/CCMN
    ...
    if (rechain_pending) {
        a64_rechain_pending_cc_after_transparent_consumer(s, &pending_snapshot);
    }
    return true;
}
                                          ...
                                          CPUState *cpu)
{
    ...
    case A64_X86_FLAGS_RAW:
        ...
        dc->a64_raw_cc_op = EX_TBFLAG_A64(tb_flags, X86_CC_OP);
        break;
    ...
    a64_clear_adjacent_host_flags(dc);
...
static void aarch64_tr_translate_insn(DisasContextBase *dcbase, CPUState *cpu)
{
    ...
    s->a64_adj_host_flags.keep = false;
    if (s->a64_pending_cc.valid && a64_cmp_trace_enabled()) {
        s->a64_pending_cc.age++;
    }
    ...
    a64_finish_adjacent_host_flags_lifetime(s);
    ...
    if (s->a64_adj_host_flags.keep) {
        s->a64_adj_host_flags.end = tcg_last_op();
    }
}
