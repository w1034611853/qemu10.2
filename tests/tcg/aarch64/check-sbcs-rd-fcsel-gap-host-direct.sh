...
log="${exe}.sbcs_rd_fcsel_gap.log"
nm_log="${exe}.sbcs_rd_fcsel_gap.nm"
rm -f "$log" "$nm_log"
...
assert_peek()
{
    ...
    rg -q "A64 cmp-pending peek producer_pc=0x${producer_addr} consumer_pc=0x${consumer_addr} age=${age} via=FCSEL.*pending" \
...
assert_record "sbcs rd fcsel cs gap1" sbcs_rd_fcsel_cs_gap1_producer
assert_peek "sbcs rd fcsel cs gap1" \
    sbcs_rd_fcsel_cs_gap1_producer sbcs_rd_fcsel_cs_gap1_consumer 2

assert_record "sbcs rd fcsel mi gap4" sbcs_rd_fcsel_mi_gap4_producer
assert_peek "sbcs rd fcsel mi gap4" \
    sbcs_rd_fcsel_mi_gap4_producer sbcs_rd_fcsel_mi_gap4_consumer 5

assert_no_use "sbcs rd fcsel bad gap" sbcs_rd_fcsel_bad_gap_producer
