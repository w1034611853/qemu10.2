...
log="${exe}.adds_rd_fcsel_gap.log"
nm_log="${exe}.adds_rd_fcsel_gap.nm"
rm -f "$log" "$nm_log"
...
assert_peek()
{
    ...
    rg -q "A64 cmp-pending peek producer_pc=0x${producer_addr} consumer_pc=0x${consumer_addr} age=${age} via=FCSEL-add-pending" \
...
assert_record "adds rd fcsel eq gap1" adds_rd_fcsel_eq_gap1_producer
assert_peek "adds rd fcsel eq gap1" \
    adds_rd_fcsel_eq_gap1_producer adds_rd_fcsel_eq_gap1_consumer 2

assert_record "adds rd fcsel vs gap4" adds_rd_fcsel_vs_gap4_producer
assert_peek "adds rd fcsel vs gap4" \
    adds_rd_fcsel_vs_gap4_producer adds_rd_fcsel_vs_gap4_consumer 5

assert_no_use "adds rd fcsel bad gap" adds_rd_fcsel_bad_gap_producer
